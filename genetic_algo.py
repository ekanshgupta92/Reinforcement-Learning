...
def evaluate(env, net):
    ...
    while True:
        ...
        obs, r, done, truncated, info = env.step(acts.data.numpy()[0])
        reward += r
        done = done | truncated
        if done:
            break
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if reward_mean > 199:
            ...
            torch.save(population[0][0].state_dict(), 'Cartpole-ga.pth')
            break

        # generate next population
        prev_population = population
        population = [population[0]]   #choose the elite 
        for _ in range(POPULATION_SIZE-1):
            parent_idx = np.random.randint(0, PARENTS_COUNT)
            parent = prev_population[parent_idx][0] #choosing the parent network
            net = mutate_parent(parent)
            fitness = evaluate(env, net)
            population.append((net, fitness))
        gen_idx += 1

    agent = Net(env.observation_space.shape[0], env.action_space.n)
    agent.load_state_dict(torch.load('Cartpole-ga.pth'))
    agent.eval()
    ...
    for i in range(10):
        while True:
            env.render()
            act_prob = agent(torch.FloatTensor([state]))
            action = act_prob.max(dim=1)[1]
            state, reward, done, truncated, info = env.step(action.data.numpy()[0])
